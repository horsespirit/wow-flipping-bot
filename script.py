...
def login_e_analizza():
    ...
    # L'URL esatto che hai trovato!
    login_url = "https://api.flippingpal.com/shared/users/login-with-token"
    ...
    try:
        ...
        # Proviamo a inviare email e password a questo nuovo endpoint
        res = requests.post(login_url, json={"email": email, "password": password}, headers=headers_base)
        
        if res.status_code != 200:
            print(f"Login fallito. Stato: {res.status_code}")
            print(f"Risposta: {res.text[:200]}")
            return
        ...
        r = requests.post(market_url, headers=headers_auth, json={"retailSellingRealms": realms, "retailBuyingRealms": realms, "userId": user_id}, timeout=600)
        data = r.json()

        html = "<html><head><meta charset='utf-8'><style>body{font-family:sans-serif;background:#121212;color:white;padding:20px} .cat-box{border:1px solid #444;margin-bottom:20px;padding:15px;border-radius:8px} .buy{color:#4caf50} .sell{color:#f44336} table{width:100%}</style></head><body>"
        ...
        with open("index.html", "w", encoding="utf-8") as f:
            f.write(html)
        print("SUCCESSO: Dashboard creata!")
